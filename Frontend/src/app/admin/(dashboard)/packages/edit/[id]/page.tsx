...
import { ArrowLeft, Save, Upload, Plus, Trash2, CheckCircle, XCircle } from "lucide-react";
...
export default function EditPackagePage({
  ...
  useEffect(() => {
    const fetchPackage = async () => {
      try {
        ...
        if (Array.isArray(data.exclusions)) setExclusions(data.exclusions);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formData.append("inclusions", JSON.stringify(inclusions.filter(Boolean)));
      formData.append("exclusions", JSON.stringify(exclusions.filter(Boolean)));
      if (imageFile) {
    ...
    <div className="p-4 sm:p-8">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Update all details including timeline, inclusions and exclusions.</p>
        </div>
      </div>
        ...
        {loadingInitial ? (
          ...
          <div className="bg-green-50 border border-green-200 rounded-2xl p-10 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button onClick={() => setSaved(false)} className="bg-white border border-slate-200 text-slate-700 font-bold px-8 py-3 rounded-xl hover:bg-slate-50">Continue Editing</button>
              <Link href="/admin/packages" className="bg-blue-600 text-white font-bold px-8 py-3 rounded-xl hover:bg-blue-700 shadow-lg shadow-blue-600/20">Back to List</Link>
            </div>
          </div>
        ) : (
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="number" value={form.price} onChange={(e) => update("price", e.target.value)} className={inputCls} />
                </div>
                <div>
                  <label className={labelCls}>Duration</label>
                  <input type="text" value={form.duration} onChange={(e) => update("duration", e.target.value)} className={inputCls} />
                </div>
                <div className="md:col-span-2">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8">
              <div className="flex items-center justify-between mb-4 border-b pb-3">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">Day-by-day itinerary</p>
                </div>
                  ...
                  {timeline.map((day, i) => (
                    <div key={i} className="border border-slate-100 rounded-xl p-4 bg-slate-50/50">
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        <input type="text" value={day.title} onChange={(e) => updateDay(i, "title", e.target.value)} className={inputCls} placeholder={`Day ${i + 1} title`} />
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8">
              ...
              <div onClick={() => document.getElementById("imageInputEdit")?.click()} className="relative border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center hover:border-blue-400 transition-colors cursor-pointer overflow-hidden min-h-[200px] flex flex-col items-center justify-center">
                {imagePreview ? (
                  <>
                    ...
                    <div className="relative z-10 flex flex-col items-center">
                      ...
                      <button className="mt-2 text-xs text-blue-600 font-bold hover:underline" onClick={(e) => { e.stopPropagation(); document.getElementById("imageInputEdit")?.click(); }}>Change Image</button>
                    </div>
                  </>
                ) : (
                  <>
                ...
                <input id="imageInputEdit" type="file" accept="image/*" onChange={handleFileChange} className="hidden" />
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              ...
              <button onClick={handleSave} disabled={loading} className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-black py-3 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-lg shadow-blue-600/20">
                {loading ? (
